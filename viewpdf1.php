...
if(isset($_POST['genpdf']))
{
    ...
    foreach ($scon->query($sql) as $row) {
        ...
        $pdf->Image("http://localhost/sjce-registrationsite-main/imageassets/$stuphotoname",50,50,30,30);

        $pdf->Image("http://localhost/sjce-registrationsite-main/imageassets/$fatphotoname",90,50,30,30);

        $pdf->Image("http://localhost/sjce-registrationsite-main/imageassets/$motphotoname",130,50,30,30);


        $pdf->Cell(10,10,'College Name',1,0,'L',false); // First header column
        $pdf->Cell(10,10,$row['clginfo'],1,0,'L',false); // Second header column
        $pdf->Ln(15);
        ...
        $pdf->Cell(10,5,'Name',1,0,'L',false); // First header column
        $pdf->Cell(10,5,$row['candidatename'],1,0,'L',false); // Second header column
        $pdf->Cell(10,5,'Initial',1,0,'L',false); // Third header column
        $pdf->Cell(10,5,$row['initial'],1,1,'L',false);
